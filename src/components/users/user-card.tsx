...
import { CONTRIBUTOR_STATUS_CONFIG } from "@/lib/constants";
...
export function UserCard({ contributor, onClick }: UserCardProps) {
  const statusConfig = CONTRIBUTOR_STATUS_CONFIG[contributor.status];
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-base font-semibold text-white">{contributor.name}</p>
        <p className="mt-1 text-[10px] uppercase tracking-[0.22em] text-muted-foreground/75">
          Balance:{" "}
          <span
            className={`font-bold tabular-nums ${
              contributor.balance >= 0 ? "text-[var(--soft-mint)]" : "text-[var(--soft-peach)]"
            }`}
          >
